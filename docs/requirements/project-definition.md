...
## Constraints
- End users must need zero extra installs to run the dashboard — backend stays stdlib Python (`scripts/usage_dashboard.py`, no FastAPI or other pip dependency), frontend ships as pre-built, committed static assets (Vite + React) — no Node/npm required at runtime.
...
- Risk: committing built frontend output (`dist/`) directly into the `cairn-dashboard` submodule is unusual repo hygiene and needs a manual sync/bump step whenever the dashboard changes — not yet automated (open question carried from the design spec, `docs/.specs/2026-08-19-dashboard-react-redesign-design.md`).

## Open Questions
- How does a future cairn release bump the `dashboard/` submodule pointer — manual for now, could fold into `release-manager` later (carried from the design spec, not resolved here).
